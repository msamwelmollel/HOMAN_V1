# -*- coding: utf-8 -*-
"""
Created on Fri Aug 16 16:04:38 2019

@author: 2427060M
"""
import sys
import numpy as np
import os
import random
import shutil
from statistics import mean
#sys.path.append('C:\\Users\\msamw\\Dropbox\\PHD - MANDELA\\Paper to write\\paper 3\\new_code_d')
from data_models.base_data_model import BaseDataModel
#from base_data_model import BaseDataModel
from data_models.deep_q import DeepNeuralNetowk
#from deep_q import DeepNeuralNetowk
import matplotlib.pyplot as plt

#%%
GAMMA = 0.95
MEMORY_SIZE = 90000000
BATCH_SIZE = 10
TRAINING_FREQUENCY = 1
TARGET_NETWORK_UPDATE_FREQUENCY = 80000
MODEL_PERSISTENCE_UPDATE_FREQUENCY = 20000
REPLAY_START_SIZE = 2000

EXPLORATION_MAX = 1
EXPLORATION_MIN = 0.1
EXPLORATION_TEST = 0.001
EXPLORATION_STEPS = 850000
EXPLORATION_DECAY = (EXPLORATION_MAX-EXPLORATION_MIN)/EXPLORATION_STEPS



class DDQNDataModel(BaseDataModel):
    def __init__(self,input_shape,action_space,model_path):
        BaseDataModel.__init__(self,input_shape, action_space)
        self.model_path = model_path
        self.ddqn = DeepNeuralNetowk(self.input_shape, action_space).model
        if os.path.isfile(self.model_path):
            self.ddqn.load_weights(self.model_path)
            
    def _save_model(self):
        self.ddqn.save_weights(self.model_path)
        
        

class DDQNSolver(DDQNDataModel):

    def __init__(self,input_shape, action_space, intensity, min_SNR, delay_cost, user):
        testing_model_path = './data_models/models/model_save_intensity_'+str(intensity)+'_min_sinr_'+str(min_SNR)+'_delay_cost_'+str(delay_cost)+'_user_'+str(user)+'_.h5'
        #"./output/neural_nets/" + game_name + "/ddqn/testing/model.h5"
        print(testing_model_path)
        if os.path.isfile(testing_model_path):
            DDQNDataModel.__init__(self, input_shape,action_space,testing_model_path)
        #assert os.path.exists(os.path.dirname(testing_model_path)), "No testing model in: " + str(testing_model_path)
        #DDQNDataModel.__init__(self, input_shape,action_space,testing_model_path)
        

    def move(self, state):
        state= (np.squeeze(state, axis=0))
        if np.random.rand() < EXPLORATION_TEST:
            return random.randrange(self.action_space)
        q_values = self.ddqn.predict(np.expand_dims(np.asarray(state).astype(np.float64), axis=0), batch_size=1)
        return np.argmax(q_values[0])
        

class DDQNTrainer(DDQNDataModel):
    def __init__(self,input_shape, action_space,model_path):
        DDQNDataModel.__init__(self, input_shape,action_space,model_path)
        
        self.model_path = model_path
        
    
        self.ddqn_target = DeepNeuralNetowk(self.input_shape, action_space).model
        self._reset_target_network()
        self.epsilon = EXPLORATION_MAX
        self.memory = []
        
        
    def move(self, state):

        state= (np.squeeze(state, axis=0))
        if np.random.rand() < self.epsilon or len(self.memory) < REPLAY_START_SIZE:
            return random.randrange(self.action_space)
        q_values = self.ddqn.predict(np.expand_dims(np.asarray(state).astype(np.float64), axis=0), batch_size=1)
        return np.argmax(q_values[0])
    
    def remember(self, current_state, action, reward, next_state, terminal):

        self.memory.append({"current_state": current_state, "action":action, "reward": reward,"next_state": next_state, "terminal": terminal})
        
        if len(self.memory) > MEMORY_SIZE:
            self.memory.pop(0)
            
    def step_update(self, total_step):
        if len(self.memory) < REPLAY_START_SIZE:
            return
        
        if total_step % TRAINING_FREQUENCY == 0 :
            loss, accuracy, average_max_q = self._train()

            
#            self.logger.add_loss(loss)
#            self.logger.add_accuracy(accuracy)
#            self.logger.add_q(average_max_q)
            
        self._update_epsilon()
        
        if total_step % MODEL_PERSISTENCE_UPDATE_FREQUENCY == 0:
                self._save_model()
                
        if total_step % TARGET_NETWORK_UPDATE_FREQUENCY == 0:
            self._reset_target_network()
            #print('metric')
            #print('metric')
            
    def _train(self):
        batch = np.asarray(random.sample(self.memory, BATCH_SIZE))
        if len(batch) < BATCH_SIZE:
            return
        
        current_states = []
        q_values = []
        max_q_values = []
        

        for entry in batch :
            current_state = np.expand_dims(np.array(np.asarray(entry["current_state"]),dtype=np.float64), axis = 0)
            current_state= (np.squeeze(current_state, axis=0))
            current_states.append(current_state)
            next_state = np.expand_dims(np.array(np.asarray(entry["next_state"]),dtype=np.float64), axis = 0)
            next_state= (np.squeeze(next_state, axis=0))

            next_state_prediction = self.ddqn_target.predict(next_state).ravel()
            next_q_value = np.max(next_state_prediction)
            q = list(self.ddqn.predict(current_state)[0])

            if entry["terminal"]:
                q[entry['action']] = entry["reward"]
                
                
            else:
                q[entry["action"]] = entry["reward"] + GAMMA * next_q_value
                
            q = np.asarray(q, dtype = np.float64)    
            q_values.append(q)
            max_q_values.append(np.max(q))
#        q_values = np.asarray(q_values, dtype= np.float64)  
        
        fit = self.ddqn.fit(np.asarray(current_states).squeeze(), np.asarray(q_values).squeeze(), batch_size = BATCH_SIZE, verbose= 0)
        loss = fit.history["loss"][0]
        #plt.plot(fit.history['loss'])
        #accuracy = fit.history["acc"][0]
        accuracy = 1
        return loss, accuracy, mean(max_q_values)
    
    def _update_epsilon(self):
        self.epsilon -= EXPLORATION_DECAY
        self.epsilon = max(EXPLORATION_MIN, self.epsilon)

    def _reset_target_network(self):
        self.ddqn_target.set_weights(self.ddqn.get_weights())                
        
            
            
            
        
            
            
            
            
            
            
            
            
            
            
        